...
| D. 具身现实感知（推荐，第一波随 A 交付） | 电量/位置/前后台 → 临桥 `/reality/context` → 人格上下文；后续主动问候+摄像头单帧（3.5 节） | A 后 +1 天 | 具身核心体验 |
...
**(3) 全功能审计：哪些值得进 banxia（具身视角，逐项源码核实）**

> 用户决策：**只做具身相关的**（让角色"活在你的现实里、关心你"的能力）。
> 米家/房间/窥屏等非具身或第三方生态的明确排除。

| # | 它的功能（源码核实） | 具身价值 | banxia 侧成本 | 决策 |
|---|---|---|---|---|
| 1 | 设备状态 `/device/status`（电量/充电/前后台，10min TTL） | ★★★ "她发现你手机快没电了" | 手机：Unity Battery API **零权限零成本** | **P0** |
| 2 | 位置上报 `/location` + 到达/离开通知（坐标+本地地点匹配+新鲜度校验） | ★★★ 通勤/到家感知 | 手机：前台服务+FINE 权限（可先做手动 COARSE） | **P0** |
| 3 | 遥测 `/telemetry`（≤32 项数值+活动+饮食/用药打卡，去重+TTL） | ★★★ 健康关怀 | 手机：Health Connect 接入较重 | P2（API 形状先行） |
| 4 | 高德逆地理增强（city/district 级模糊标签，百米缓存，失败静默） | ★★ "今天一直在徐汇附近" | 服务端可选（用户有 Key 才开） | P2（抄其模糊注入设计） |
| 5 | **主动语音/提醒/起床闹钟**（audio_only/audio_and_chat/chat_on_failure 三投递模式，主动语音镜像到设备） | ★★★ **她主动开口** | 手机：**SSE 下行通道已有**+TTS 原生播放；服务端：触发时机 | **P1** |
| 6 | **摄像头单帧**（明确目的、单帧不落盘、失败如实回执 `must_not_claim_observed`、主动好奇默认关+分层授权+日限额） | ★★★★ "看看我今天穿的" | 手机：WebCamTexture 单帧 JPEG（随身相机，比它的 PC 摄像头强） | **P1**（隐私设计整套抄） |
| 7 | 伪窥屏（UsageStats 应用类别脱敏摘要，低置信度语气） | ★（它自己都极保守） | — | 不做 |
| 8 | 米家控制（凭证独立、别名校验、扫码授权） | ★★ 她开灯（需用户有米家） | —（服务端功能） | 不做——装它插件它自己就有，不重复造轮 |
| 9 | 房间代理/一起看/屏幕共享（Together/Game/Screen Companion 联动） | 0（非具身生态） | — | 不做 |
| 10 | 治理设计（授权分能力可撤销、确认对话、结构化失败回执、上下文模糊注入） | 设计模式本身 | — | **全部照抄为设计规范** |

**架构决策：走临桥（astrbot_plugin_embodiment_bridge），不接它的网关**

banxia 手机端 → 临桥新增 `POST /reality/context`，理由：
1. 硬约束契合：临桥是凝心溯溪系列自有插件，随便改；reality_companion 只参考
2. 一条通道一条鉴权：对话+感知同一 pipe（现有持久双钥 + session_id），
   不引入第二套配对令牌/会话模型
3. **注入链路现成**：临桥 `/spatial/context` 已实现
   `StrictModel 校验 → session 内存态 → <embodiment_spatial_context_json> overlay
   注入对话上下文`，`/reality/context` 同构复制这条链路（已核实 main.py L113
   `_build_spatial_context_overlay` 与 session_manager L894）
4. 它的网关绑定它自己的插件族（Private Companion tier 授权/人格注入）——
   未装那套时数据收了也无人消费
5. 主动语音不需要它的 aiohttp 独立网关——临桥 SSE 事件流就是下行通道

**三条隐私设计红线（照抄它的实现）**：
- 注入人格上下文的是**模糊语义**（"ta 在外/电量低"），不是裸坐标/原始数值
- 能力级授权、随时可撤（手机端位置共享开关）
- 摄像头失败**如实回执**——角色不得编造看到的画面

**分波实施（具身感知闭环）**：

- **第一波（最小闭环）**：手机端 `PhoneRealityContextReporter`（~200 行 C#：
  电量/充电/前后台 + 手动位置刷新，零/低权限）→ 临桥 `/reality/context`
  （~300 行：RealityContextRequest/Snapshot + TTL 内存态 + overlay 注入）
- **第二波**：前台服务持续位置 + 到达/离开通知；SSE 主动问候/提醒下行事件；
  摄像头单帧（走多模态对话管线）
- **第三波**：Health Connect 遥测、高德逆地理（可选）
