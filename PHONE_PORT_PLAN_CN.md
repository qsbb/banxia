...
## M7 P3 双端回归登记（场景加载进度与加载门）

- Unity `RuntimeMmdModelLoader` 与 Flutter `model.loadProgress` 使用同一套请求 ID/代次，手机与 Quest 共享，不在平台分支中复制业务逻辑。
- 加载门必须覆盖场景手势、场景 HUD/弹窗及底部导航；`fraction=1` 仍等待 `copresence.mode.inScene=true` 才关闭。
- 手机回归：进入场景、快速返回/取消、失败后返回首页、旧代次进度不改变当前门状态。
- Quest 回归（待设备窗口）：同样验证加载/取消/失败/成功四条路径；仅执行应用启动/停止，不执行重启或设置命令。
