...
> **2026-09-07 终审更正**：本文"蒙皮网格纵向压缩 y′≈0.507y+0.92"的结论经
> 同刻探针（BakeMesh 逐顶点 + 导演相机位姿 + 截图像素三方对照）**证伪**——
> 蒙皮网格在世界空间是完整站立的（bakedWorldY=[-0.004,1.676]，接地、全高）。
> 真实根因是**渲染相机从未到达求解位姿**：场景 XR 遗留的
> `UnityEngine.InputSystem.XR.TrackedPoseDriver` 在无 XR 运行时的手机端
> 每帧把主相机姿态重置为默认值（原点恒等），覆盖轨道相机的 ApplyTransform。
> 此前所有"渲染高度"反推都建立在"相机在求解位姿"的错误前提上，故得出
> 虚假的"压缩"拟合。修复：手机端引导时按类型名禁用该组件
> （`QuestMmdPlayerBootstrap.DisableXrPoseDrivers`，f3cfd03）。修复后同刻
> 复核：相机位姿=求解值（h=1.377/d=0.806），头像胸像构图正确（脸≈40% 屏高）。
> 下文"压缩"证据链仅作历史记录保留，其测量方法（错相机反推）即为假象来源。
...
| # | 问题 | 根因状态 | 建议 |
|---|---|---|---|
| 1 | ~~蒙皮渲染纵向压缩~~ → **实为渲染相机被 XR TrackedPoseDriver 逐帧重置到原点**（终审更正见文首） | ✅ 已定位已修复（f3cfd03：手机端引导禁用姿态驱动；同刻三方对照复核通过） | — |
| 1b | **最小化（moveTaskToBack）后 resume 渲染全白、fps=0、进程内不可恢复** | ✅ 已定位：Unity 主线程直接调 Activity.moveTaskToBack 破坏 SurfaceView 事务时序；改 runOnUiThread（3f63e3b） | 验证：minimize→relaunch 后渲染存活 |
| 2 | 返回键全局无效 | ✅ 已定位已修复（Java 拦截层 + Dart handleSystemBack 链：sheet→场景→导航→tab→最小化；像素级验证通过） | — |
| 3 | 场景内 Unity 触摸全灭 | ✅ 已定位已修复（_SceneGestureLayer 桥接 orbit/zoom/pan/double-tap） | 手势像素验证进行中 |
| 4 | 物理：生成瞬间裙摆爆炸（+3s 截屏见大范围对角拖尾）；`pose_src_flip` 在 60Hz 档下 ~19/s 持续上涨（姿态缓存持续失效） | 未定位；与 #1 不同源（#1 已证伪蒙皮发散） | 探针顺带记录物理体初始世界位置 |
| 5 | 构图网格/诊断 HUD 设置不跨启动恢复 | ✅ 已修复并验证：settings.state 事件下发 + Flutter 侧持久化，杀进程重启后开关状态恢复（debug-mode 像素验证通过） | — |
| 6 | 底部胶囊/导航无障碍边界 [0,0][0,0]（TalkBack 失效；点击正常） | 面板窗口 a11y 视口（[0,136][1080,2208]）与实际全屏绘制不一致 | 低优先 |
| 7 | VMD 播放前同步转换 19–21s（一个 4 帧 VMD 烘焙出 228387 keys / 387 条曲线），期间 UI 无反馈；进场景必重载模型并杀掉播放 | ✅ 机制清楚；进场景免重载已修（d2e8939） | 转换异步化+进度；动作页内直接预览 |
