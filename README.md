...
- Quest 性能页读取 OpenXR App CPU/GPU、合成器丢帧和佩戴会话帧率，并区分离头限频与真实渲染压力。
- 渲染画质与 MMD 物理独立调节；物理提供性能 60Hz/2 子步、平衡 60Hz/2 子步和精细 120Hz/4 子步，离头/失焦时暂停推进，避免物理追赶和累计统计污染。
...
VMD 首次播放完成转换后会在后台保存版本化曲线缓存；后续 App 启动可跳过同一模型与动作的 Bullet/IK 烘焙。缓存按 PMX/VMD 内容和转换参数自动失效，并限制为 256 MiB、最多 8 项；损坏或空间不足时只会回退正常转换，不影响原动作文件。

后端动作使用 `action_id + method + parameters + transition + source` 的受限方法调用。客户端会在建连时声明实际支持的动作；当前新增的 `crouch` 使用髋、膝、踝和固定脚目标完成自然下蹲，缺少必要腿部骨骼时会明确拒绝，而不是口头假装已经执行。旧版后端仍可继续发送只有 `gesture` 的兼容事件。
